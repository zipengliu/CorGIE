                        ...
                        <g className="bar-charts" transform={`translate(${dotsWidth},0)`}>
                            {neighborIntersections.map((plotData, i) =>
                                <g key={i} transform={`translate(${i * (cardScaleRange + plotHorizontalMargin)},0)`}>
                                    <g className="axis">
                                        <text x={0} y={topLabelHeight - 4} transform={`rotate(-15,0,${topLabelHeight-4})`}>
                                            {nodeTypes[i].name}
                                        </text>
                                        <line x1={0} y1={topLabelHeight - 2} x2={cardScaleRange + 2} y2={topLabelHeight - 2}
                                              markerEnd="url(#arrow)"/>
                                    </g>
                                    <g transform={`translate(0,${topLabelHeight})`}>
                                        {plotData.map((row, j) =>
                                            <g key={j} transform={`translate(0,${j * (dotSize + verticalMargin)})`}>
                                                <rect className="bar" x={0} y={0}
                                                      height={dotSize} width={scalesByType[i](row.size)}
                                                      style={{fill: nodeTypes[i].color}} />
                                                {row.size > 0 &&
                                                <text x={scalesByType[i](row.size) + 2} y={8}>{row.size}</text>}
                                            </g>
                                        )}
                                    </g>
                                </g>

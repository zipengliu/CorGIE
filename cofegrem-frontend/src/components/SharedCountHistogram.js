...
class SharedCountHistogram extends Component {
    ...
    render() {
        ...
        const yScale = scaleLinear().domain([0, neighborCounts.length]).range([0, height]);
            ...
            <div>
                <svg width={svgWidth} height={svgHeight}>
                    <g transform={`translate(${margins.left},${margins.top})`}>
                        {neighborCountsBins.map((bins, i) =>
                            bins.length > 0 &&
                            <g key={i} transform={`translate(${histoXPos[i]},0)`}>
                                {bins.map((b, j) =>
                                    b.length > 0 &&
                                    <g key={j} transform={`translate(${j*(barWidth+barGap) + barGap},${height - yScale(b.length)})`}>
                                        <rect style={{stroke: 'none', fill: nodeTypes[i].color}}
                                              x={0} y={0}
                                              width={barWidth} height={yScale(b.length)} />
                                        <text x={0} y={-5}>{b.length}</text>
                                        <text x={0} y={yScale(b.length) + 10}>{b[0]}</text>
                                    </g>
                                )}
                                <g className='axis'>
                                    <line x1={0} y1={height} x2={0} y2={-5} markerEnd="url(#arrow)"
                                          style={{strokeWidth: '1px', stroke: '#000'}} />
                                </g>
                                <g className='axis'>
                                    <line x1={0} y1={height} x2={bins.length*(barWidth+barGap)} y2={height}
                                          markerEnd="url(#arrow)"
                                          style={{strokeWidth: '1px', stroke: '#000'}} />
                                </g>
                            </g>

        ...
        return (
            <div id="adjacency-matrix-view" className="view">
                ...
                <div style={{ fontSize: "14px" }}>
                    (row: neighbor sets of selected nodes; column: neighbor nodes sorted by node type)
                </div>
                <svg width={svgWidth} height={svgHeight}>
                    <g transform={`translate(${margins.left},${margins.top})`}>
                        ...
                        <g
                            className="labels"
                            transform={`translate(${labelAreaSize - labelSize / 2 - 4},${labelAreaSize +
                                labelSize / 2})`}
                        >
                            {selectedNodes.map((id, i) => (
                                <g key={i} transform={`translate(0,${i * (rowHeight + gap)})`}>
                                    <circle
                                        className={cn("node selected", {
                        ...
                        <g
                            className="labels"
                            transform={`translate(${labelAreaSize + labelSize / 2},${labelAreaSize -
                                labelSize / 2 -
                                4})`}
                        >
                        ...
                        {/*last row: total*/}
                        <g
                            transform={`translate(${labelAreaSize},${labelAreaSize +
                                selectedNodes.length * (rowHeight + gap)})`}
                        >
                            {neighborCounts.map((neigh, j) => (
                        ...
                        <g
                            transform={`translate(${labelAreaSize +
                                neighborCounts.length * (colWidth + gap)},${labelAreaSize})`}
                        >
                            {selectedNodes.map((id, i) => (
                ...
                <h5>
                    Histogram of frequencies of neighbor nodes in selected neighbor sets (counts of counts)
                </h5>
